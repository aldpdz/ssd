...
def ssd_300(image_size,
    ...
    def liteConv(inputs, filters, alpha, block_id=11):
        channel_axis = -1
        filters = int(filters * alpha)
        x = Conv2D(filters, (1, 1), padding='same', use_bias=False, name=str(block_id)+'_pwconv1')(inputs)
        x = Lambda(coord_conv, output_shape=(x._keras_shape[1], x._keras_shape[2], x._keras_shape[3] + 2), name=str(block_id)+'_conv_coord')(x)
    ...
    def coord_conv(tensor):
        batch_size_tensor = tf.shape(tensor)[0] 
        y_dim = tensor._keras_shape[1]
        x_dim = tensor._keras_shape[2]
        ...
        ret = tf.concat([tensor, xx_channel, yy_channel], axis=-1)
        return ret
    ...
    # Add coordlayer to input
    x1 = Lambda(coord_conv, output_shape=(img_height, img_width, img_channels + 2), name='input_coord_conv')(x1)
   
    layer = _conv_block(x1, 32, alpha, strides=(2, 2))
    layer = _inverted_res_block(layer, filters=16, alpha=alpha, stride=1,expansion=1, block_id=0)
    layer = _inverted_res_block(layer, filters=24, alpha=alpha, stride=2,expansion=6, block_id=1)
    layer = _inverted_res_block(layer, filters=24, alpha=alpha, stride=1,expansion=6, block_id=2)
    layer = _inverted_res_block(layer, filters=32, alpha=alpha, stride=2,expansion=6, block_id=3)
    layer = _inverted_res_block(layer, filters=32, alpha=alpha, stride=1,expansion=6, block_id=4)
    layer = _inverted_res_block(layer, filters=32, alpha=alpha, stride=1,expansion=6, block_id=5)
    layer = _inverted_res_block(layer, filters=64, alpha=alpha, stride=2,expansion=6, block_id=6)
    layer = _inverted_res_block(layer, filters=64, alpha=alpha, stride=1,expansion=6, block_id=7)
    layer = _inverted_res_block(layer, filters=64, alpha=alpha, stride=1,expansion=6, block_id=8)
    layer = _inverted_res_block(layer, filters=64, alpha=alpha, stride=1,expansion=6, block_id=9)
    layer = _inverted_res_block(layer, filters=96, alpha=alpha, stride=1,expansion=6, block_id=10)
    layer = _inverted_res_block(layer, filters=96, alpha=alpha, stride=1,expansion=6, block_id=11)
    layer = _inverted_res_block(layer, filters=96, alpha=alpha, stride=1,expansion=6, block_id=12)
    layer, conv13 = _isb4conv13(layer, filters=160, alpha=alpha, stride=1,expansion=6, block_id=13)
    layer = _inverted_res_block(layer, filters=160, alpha=alpha, stride=1,expansion=6, block_id=14)
    layer = _inverted_res_block(layer, filters=160, alpha=alpha, stride=1,expansion=6, block_id=15)
    layer = _inverted_res_block(layer, filters=320, alpha=alpha, stride=1,expansion=6, block_id=16)    
    layer, conv17 = Conv(layer, 1280)
    layer, conv18_2 = liteConv(layer, 256, alpha, block_id=18)
    layer, conv19_2 = liteConv(layer, 128, alpha, block_id=19)
    layer, conv20_2 = liteConv(layer, 128, alpha, block_id=20)
    layer, conv21_2 = liteConv(layer, 64, alpha, block_id=21)

    ## Add coord_conv to predictions layers
    conv13 = Lambda(coord_conv, output_shape=(conv13._keras_shape[1], conv13._keras_shape[2], conv13._keras_shape[3] + 2), name='coord_conv_conv13')(conv13)
    conv17 = Lambda(coord_conv, output_shape=(conv17._keras_shape[1], conv17._keras_shape[2], conv17._keras_shape[3] + 2), name='coord_conv_conv17')(conv17)
    # conv18_2 = Lambda(coord_conv, output_shape=(conv18_2._keras_shape[1], conv18_2._keras_shape[2], conv18_2._keras_shape[3] + 2), name='coord_conv_conv18')(conv18_2)
    # conv19_2 = Lambda(coord_conv, output_shape=(conv19_2._keras_shape[1], conv19_2._keras_shape[2], conv19_2._keras_shape[3] + 2), name='coord_conv_conv19')(conv19_2)
    # conv20_2 = Lambda(coord_conv, output_shape=(conv20_2._keras_shape[1], conv20_2._keras_shape[2], conv20_2._keras_shape[3] + 2), name='coord_conv_conv20')(conv20_2)
    #conv21_2 = Lambda(coord_conv, output_shape=(conv21_2._keras_shape[1], conv21_2._keras_shape[2], conv21_2._keras_shape[3] + 2), name='coord_conv_conv21')(conv21_2)
